<div class="row post-preview mb-3">
    ...
    <div class="vote-buttons col-md-4 d-flex justify-content-end align-items-center">
        <button class="btn btn--vote btn-success mr-1" data-phrase-name="<?= get_the_title(); ?>" data-phrase-id="<?= get_the_ID(); ?>" data-vote-value="1"><?= __('Znam'); ?></button>
        <button class="btn btn--vote btn-danger" data-phrase-name="<?= get_the_title(); ?>" data-phrase-id="<?= get_the_ID(); ?>" data-vote-value="-1"><?= __('Nie znam'); ?></button>
    </div>
</div>